...
void main(void)
{
    ...
    // debugging camera
    float mx=iMouse.x/iResolution.x*PI*2.0 + iGlobalTime * 0.166;
    float my=-iMouse.y/iResolution.y*10.0 + sin(iGlobalTime * 0.3)*0.8+0.1;//*PI/2.01;
    // move camera in and out of the sphere
    float smallTime = iGlobalTime*0.2;
    ...
    // -------------------------------- animate ---------------------------------------
    float localTime = iGlobalTime*0.5;
