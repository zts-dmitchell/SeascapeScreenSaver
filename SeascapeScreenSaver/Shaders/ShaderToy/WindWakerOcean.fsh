// "Wind Waker Ocean" by @Polyflare/@UnitVec (29/1/15)
...
{
    c = abs(pos - c);
    c.x = c.x > 0.5 ? 1.0 - c.x : c.x;
    c.y = c.y > 0.5 ? 1.0 - c.y : c.y;
#if FAST_CIRCLES
...
vec3 water(vec2 uv, vec3 cdir)
{
    ...
    d1 = iGlobalTime * 0.07 + d1;
    d2 = iGlobalTime * 0.5 + d2;
    vec2 dist = vec2(
...
#if ANIMATE_CAM
            // Rotating camera
            float cost = cos(iGlobalTime * -0.05);
            float sint = sin(iGlobalTime * -0.05);
