...
vec3 Light(vec3 P, vec3 rd, float t, float d){
    ...
    float s=0.03;
    if(clr.b>0.15)s=0.8;
    if(spec<0.1)N=normalize(N+(texture2D(tex,P.xz*s).rgb-vec3(0.5))*0.2);
...
void main() {
    ...
    vec3 rd=lookat(cp-ro,vec3(0.0,1.0,0.0))*normalize(vec3((2.0*gl_FragCoord.xy-size.xy)/size.y,12.0));
